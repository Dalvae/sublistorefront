...
const DiscountCode: React.FC<DiscountCodeProps> = ({ cart }) => {
  ...
  const appliedDiscount = useMemo(() => {
    ...
    switch (discounts[0].rule.type) {
      ...
      case "fixed":
        const adjustedDiscountValue = adjustPriceForZeroDecimalCurrency(
          discounts[0].rule.value,
          region.currency_code
        )
        return `- ${formatAmount({
          amount: adjustedDiscountValue,
          region: region,
          locale: "es-CL",
        ...
        {appliedDiscount ? (
              ...
              <div className="flex items-center justify-between w-full max-w-full">
                ...
                >
                  ...
                  <span className="sr-only">
                    Remover codigo de descuento de la orden
                  </span>
                </button>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit(onApply)} className="w-full">
            <Label className="flex gap-x-1 mb-2">
              Gift card o codigo de descuento?
